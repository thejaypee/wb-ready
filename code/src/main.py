...
if st.session_state.analysis and st.session_state.repo_info:
    ...
    st.markdown(
        "This creates a **fresh Workbench-ready repo** on your GitHub with the latest files "
        "and `.project/` config. No git history bloat - fast Workbench cloning. Original repo is never modified."
    )
    ...
    if st.button("Convert & Push", type="primary"):
        ...
        try:
            ...
            status.text("Cloning repository (shallow)...")
            ...
            try:
                ...
                # Create a new repo with just the latest files (no history bloat)
                new_url = client.create_fresh_repo(repo_info.owner, repo_info.name, clone_path)

                status.text("Pushing to GitHub...")
                progress.progress(75)

                msg = (
                    "NVIDIA AI Workbench-ready project\n\n"
                    f"Converted from {repo_info.full_name} by WB-Ready.\n"
                    "Includes .project/ configuration for direct Workbench import.\n"
                )
                client.push_changes(clone_path, msg, "main")
            finally:
                shutil.rmtree(tmp, ignore_errors=True)
            ...
            repo_web = new_url.replace(".git", "")
            st.success(f"Done! Workbench-ready repo: {repo_web}")
            st.markdown(f"Clone this URL in Workbench: `{new_url}`")
            st.balloons()
